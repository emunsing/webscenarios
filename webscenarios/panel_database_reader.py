...
class Project(Base):
    ...
    configuration_text = Column(Text, nullable=False, default="")
...
async def create_project(project_data: ProjectCreate):
    ...
    async with async_session_maker() as db_session:
        project = Project(
            ...
            configuration_name=project_data.configuration_name,
            configuration_text=project_data.configuration_text
        )
...
async def update_project(project_id: uuid.UUID, project_data: ProjectUpdate):
    ...
    async with async_session_maker() as db_session:
        ...
        if project_data.configuration_name is not None:
            project.configuration_name = project_data.configuration_name
        if project_data.configuration_text is not None:
            project.configuration_text = project_data.configuration_text
...
def create_panel_app():
    ...
    configuration_name_input = pn.widgets.TextInput(name="Configuration Name", value="", width=400)
    config_text_input = pn.widgets.TextAreaInput(name="Configuration Text", value="", height=150, width=400)
    ...
    async def save_project(event):
        ...
        config_name = configuration_name_input.value
        ...
        if not organization:
            ...
            return
        if not config_name:
            status_text.object = "❌ Configuration name is required"
            return
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    if projects:
                        ...
                        table_rows = [
                            "| ID | Name | Organization | Configuration Name | Created |",
                            "|-----|------|--------------|-------------------|---------|"
                        ]
                        for p in projects:
                            ...
                            config_name = p.get("configuration_name", "N/A")
